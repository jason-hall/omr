...
MM_CopyScanCacheChunkVLHGC::newInstance(MM_EnvironmentVLHGC*env, UDATA cacheEntryCount, MM_CopyScanCacheVLHGC **nextCacheAddr, MM_CopyScanCacheChunkVLHGC *nextChunk)
{
	...
	chunk = (MM_CopyScanCacheChunkVLHGC *)env->getForge()->allocate(sizeof(MM_CopyScanCacheChunkVLHGC) + cacheEntryCount * sizeof(MM_CopyScanCacheVLHGC), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (chunk) {
