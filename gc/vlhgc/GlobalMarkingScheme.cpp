...
MM_GlobalMarkingScheme::newInstance(MM_EnvironmentVLHGC *env)
{
	...
	markingScheme = (MM_GlobalMarkingScheme *)env->getForge()->allocate(sizeof(MM_GlobalMarkingScheme), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (markingScheme) {
...
void 
MM_GlobalMarkingScheme::scanClassLoaderObject(MM_EnvironmentVLHGC *env, fomrobject_t *classLoaderObject, ScanReason reason)
{
...
void
MM_GlobalMarkingScheme::processReferenceList(MM_EnvironmentVLHGC *env, fomrobject_t* headOfList, MM_ReferenceStats *referenceStats)
{
