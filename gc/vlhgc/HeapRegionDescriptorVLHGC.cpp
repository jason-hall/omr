...
MM_HeapRegionDescriptorVLHGC::initialize(MM_EnvironmentBase *env, MM_HeapRegionManager *regionManager)
{
	...
	/* OMRTODO _unfinalizedObjectList.setNextList(extensions->unfinalizedObjectLists);
	...
	/* OMRTODO _ownableSynchronizerObjectList.setNextList(extensions->ownableSynchronizerObjectLists);
...
MM_HeapRegionDescriptorVLHGC::tearDown(MM_EnvironmentBase *env)
{
	...
	// OMRTODO extensions->unfinalizedObjectLists = NULL;
...
{
	return MM_GCExtensionsBase::getExtensions(env->getOmrVM())->interRegionRememberedSet->allocateRegionBuffers((MM_EnvironmentVLHGC *)env, this);
}
