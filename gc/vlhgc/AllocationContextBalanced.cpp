...
{
	MM_AllocationContextBalanced *context = (MM_AllocationContextBalanced *)env->getForge()->allocate(sizeof(MM_AllocationContextBalanced), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (context) {
...
MM_AllocationContextBalanced::initialize(MM_EnvironmentBase *env)
{
	...
	_freeProcessorNodes = (UDATA *)env->getForge()->allocate(arraySizeInBytes, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
MM_AllocationContextBalanced::lockedAllocateArrayletLeaf(MM_EnvironmentBase *envBase, MM_AllocateDescription *allocateDescription, MM_HeapRegionDescriptorVLHGC *freeRegionForArrayletLeaf)
{
	...
	fomrobject_t *spine = allocateDescription->getSpine();
