...
{
	MM_ConfigurationIncrementalGenerational *configuration = (MM_ConfigurationIncrementalGenerational *) env->getForge()->allocate(sizeof(MM_ConfigurationIncrementalGenerational), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
MM_ConfigurationIncrementalGenerational::createDefaultMemorySpace(MM_EnvironmentBase *env, MM_Heap *heap, MM_InitializationParameters *parameters)
{
	...
	extensions->rememberedSetCardBucketPool = (MM_RememberedSetCardBucket *)extensions->getForge()->allocate(allocateSize, MM_AllocationCategory::REMEMBERED_SET, OMR_GET_CALLSITE());
...
{
	MM_EnvironmentVLHGC env((OMR_VM *)extensions->getOmrVM()->_language_vm);
...
MM_ConfigurationIncrementalGenerational::initialize(MM_EnvironmentBase *env)
{
	...
	if (result) {
		extensions->scavengerScanOrdering = MM_GCExtensionsBase::OMR_GC_SCAVENGER_SCANORDERING_BREADTH_FIRST;
