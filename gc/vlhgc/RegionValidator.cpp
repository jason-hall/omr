...
MM_RegionValidator::validate(MM_EnvironmentBase *env) 
{
	...
	if (MM_HeapRegionDescriptor::BUMP_ALLOCATED == regionType) {
		...
		MM_HeapMapWordIterator firstWordIterator(MM_GCExtensionsBase::getExtensions(env)->previousMarkMap, _region->getLowAddress());
		fomrobject_t *firstObject = firstWordIterator.nextObject();
		if (NULL != firstObject) {
			J9Class *clazz = J9GC_J9OBJECT_CLAZZ(firstObject);
			if (NULL == clazz) {
