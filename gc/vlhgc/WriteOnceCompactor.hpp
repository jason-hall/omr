...
class MM_WriteOnceCompactor : public MM_BaseVirtual
{
	/* Data Members */
private:
    OMR_VM * const _javaVM;	/**< Cached pointer to the common JavaVM instance */
    MM_GCExtensionsBase * const _extensions;	/**< Cached pointer to the common GCExtensions instance */
...
private:
	 ...
	 */
	UDATA bytesAfterSlidingTargetToLocateObject(fomrobject_t *oldPointer, fomrobject_t *slidingTarget) const;
	 ...
	 */
	fomrobject_t *getForwardWrapper(MM_EnvironmentVLHGC *env, fomrobject_t *objectPtr, J9MM_FixupCache *cache);
    ...
    void saveForwardingPtr(fomrobject_t* objectPtr, fomrobject_t* forwardingPtr);
	 ...
	 */
	void fixupMixedObject(MM_EnvironmentVLHGC* env, fomrobject_t *objectPtr, J9MM_FixupCache *cache);
#if 0 OMRTODO
	void fixupClassObject(MM_EnvironmentVLHGC* env, fomrobject_t *classObject, J9MM_FixupCache *cache);
	void fixupClassLoaderObject(MM_EnvironmentVLHGC* env, fomrobject_t *classLoaderObject, J9MM_FixupCache *cache);
#endif
	void fixupPointerArrayObject(MM_EnvironmentVLHGC* env, fomrobject_t *objectPtr, J9MM_FixupCache *cache);
#if 0
     ...
     */
    MMINLINE IDATA pageIndex(fomrobject_t* objectPtr) const
    {
	 ...
	 */
    MMINLINE fomrobject_t* pageStart(UDATA i) const
    {
        return (fomrobject_t*) ((UDATA)_heapBase + (i * sizeof_page));
    }
	 ...
	 */
	void verifyHeapMixedObject(fomrobject_t* objectPtr);

	/**
	 * Verify array object in heap
	 * @param objectPtr pointer to object for fixup
	 */
	void verifyHeapArrayObject(fomrobject_t* objectPtr);

	/**
	 * Verify object slot
	 * @param object pointer to object slot
	 */
	void verifyHeapObjectSlot(fomrobject_t* object);
	 ...
	 * @param byteCount[in/out] The number of bytes the receiver planned to move
	 */
	fomrobject_t *doPlanEvacuate(MM_EnvironmentVLHGC *env, void **copyDestinationBase, void *copyDestinationTop, fomrobject_t *firstToCopy, fomrobject_t *endOfCopyBlock, UDATA *objectCount, UDATA *byteCount);
	/**
	 * Part of planning refactoring (JAZZ 21595).
	 * This function is analogous to doCompact.  It is meant to be the non-moving equivalent which only performs planning actions
	 * but does not move objects or write to the heap.
	 ...
	 * @param byteCount[in/out] The number of bytes the receiver planned to move
	 */
	void doPlanSlide(MM_EnvironmentVLHGC *env, void *copyDestinationBase, fomrobject_t *firstToCopy, fomrobject_t *endOfCopyBlock, UDATA *objectCount, UDATA *byteCount);
	 ...
	 */
	void evacuatePage(MM_EnvironmentVLHGC *env, void *page, J9MM_FixupTuple *previousTwoObjects, fomrobject_t **newLocationPtr);
	...
	void fixupObject(MM_EnvironmentVLHGC *env, fomrobject_t *objectPtr, J9MM_FixupCache *cache);
	 ...
	 */
	MMINLINE void addOwnableSynchronizerObjectInList(MM_EnvironmentVLHGC *env, fomrobject_t *objectPtr)
	{
	 ...
	 */
	void fixupFinalizableList(MM_EnvironmentVLHGC *env, fomrobject_t headObject);
	 ...
	 * @param env[in] A GC thread
	 */
	// OMRTODO void rememberClassLoaders(MM_EnvironmentVLHGC *env);
	
	/**
	 * At the end of a compaction, restore the next mark bits for any class loaders in the compact set.
	 * @param env[in] A GC thread
	 */
	// OMRTODO void rebuildNextMarkMapFromClassLoaders(MM_EnvironmentVLHGC *env);
	 ...
	 * @param env[in] A GC thread 
	 */
	// OMRTODO void clearClassLoaderRememberedSetsForCompactSet(MM_EnvironmentVLHGC *env);
...
public:
	...
	fomrobject_t *getForwardingPtr(fomrobject_t *objectPtr) const;
