...
{
	MM_WriteOnceCompactor *compactScheme = (MM_WriteOnceCompactor *)env->getForge()->allocate(sizeof(MM_WriteOnceCompactor), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
private:
	fomrobject_t *_destinationAddress;	/**< The base address of the contiguous area to which the objects in this page are moving. */
...
private:
	 ...
	 */
	UDATA
	growBitOffset(fomrobject_t* objectPtr) const
	{
		/**
...
public:
	void
	initialize(fomrobject_t* addr)
	{
		Assert_MM_false(0x1 == ((UDATA)addr & 0x1));
		_destinationAddress = (fomrobject_t *)addr;
	}
	...
	setEstimatedSize(UDATA size)
	{
		...
		_destinationAddress = (fomrobject_t *)(size | 0x1);
	}
	 ...
	 */
	bool
	isGrowBitSet(fomrobject_t *objectPtr)
	{
	 ...
	 */
	UDATA
	growBitsInfluencingObject(fomrobject_t *object)
	{
	 ...
	 */
	void
	setGrowBit(fomrobject_t *objectPtr)
	{
		UDATA offset = growBitOffset(objectPtr);
	 ...
	 */
	void
	clearGrowBit(fomrobject_t *objectPtr)
	{
		UDATA offset = growBitOffset(objectPtr);
...
{
	fomrobject_t *_oldPointer;	/**< The old object location */
	fomrobject_t *_newPointer;	/**< The new (or "fixed up") location of the same object */
...
MM_WriteOnceCompactor::getForwardWrapper(MM_EnvironmentVLHGC *env, fomrobject_t *objectPtr, J9MM_FixupCache *cache)
{
	fomrobject_t *forwarded = NULL;
	if (NULL != cache) {
...
MM_WriteOnceCompactor::compact(MM_EnvironmentVLHGC *env)
{
	...
	if (NULL != stateToFixup) {
		...
		// OMRTODO rebuildNextMarkMapFromClassLoaders(env);
			...
			if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				...
				while (NULL != (slot = slotIterator.nextSlot())) {
					...
					if (PACKET_INVALID_OBJECT != (UDATA)pointer) {
						...
						if (region->_compactData._shouldCompact) {
							fomrobject_t *newPointer = getForwardingPtr(pointer);
	...
	while (NULL != packet) {
		if (!packet->isEmpty()) {
			...
			if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				...
				fomrobject_t **slot = slotIterator.nextSlot();
				while (NULL != slot) {
					fomrobject_t *pointer = *slot;
					if (PACKET_INVALID_OBJECT != (UDATA)pointer) {
						MM_HeapRegionDescriptorVLHGC *region  = (MM_HeapRegionDescriptorVLHGC *)_regionManager->tableDescriptorForAddress(pointer);
						...
						// OMRTODO Assert_MM_mustBeClass(J9GC_J9OBJECT_CLAZZ(pointer));
						if (region->_compactData._shouldCompact) {
...
void
MM_WriteOnceCompactor::evacuatePage(MM_EnvironmentVLHGC *env, void *page, J9MM_FixupTuple *previousTwoObjects, fomrobject_t **newLocationPtr)
{
	fomrobject_t *newLocation = *newLocationPtr;
	...
	for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
		...
		fomrobject_t *objectPtr = markedObjectIterator.nextObject();
		if ((NULL == newLocation) && (NULL != objectPtr)) {
			newLocation = getForwardingPtr(objectPtr);
		}
		while (NULL != objectPtr) {
			// OMRTODO Assert_MM_mustBeClass(J9GC_J9OBJECT_CLAZZ(objectPtr));
			...
			fomrobject_t *nextObject = markedObjectIterator.nextObject();
			UDATA objectSize = _extensions->objectModel.getConsumedSizeInBytesWithHeader(objectPtr);
			fomrobject_t *nextLocation = (fomrobject_t *)((UDATA)newLocation + objectSize);
			if (newLocation != objectPtr) {
				...
				if(_extensions->objectModel.isIndexable(newLocation)) {
					updateInternalLeafPointersAfterCopy((fomrobject_t*) newLocation, (fomrobject_t*) objectPtr);
				}
#endif /* J9VM_GC_ARRAYLETS */
				if (hasBeenHashed && !hasBeenMoved) {
					/* add the hash */
				...
				nextLocation = (fomrobject_t *)((UDATA)newLocation + objectSizeAfterMove);
			}
			/* fixup this object's fields, whether it moved or not */
			fomrobject_t *toFixup = oldObject._newPointer;
...
{
	fomrobject_t *fixup = previousObjects[1]._newPointer;
	...
	while (NULL != (region = popWork(env))) {
		if (region->_compactData._shouldCompact) {
			...
			for (page = startAddress; (NULL == earlyExit) && (page < high); page = (void *)((UDATA)page + sizeof_page)) {
				...
				IDATA evacuateIndex = pageIndex((fomrobject_t *)page);
 ...
 */
MMINLINE void
MM_WriteOnceCompactor::saveForwardingPtr(fomrobject_t *objectPtr, fomrobject_t *forwardingPtr)
{
...
MM_WriteOnceCompactor::tailMarkObjectsInRegion(MM_EnvironmentVLHGC *env, MM_HeapRegionDescriptorVLHGC *region)
{
	...
	while (NULL != (walk = iterator.nextObject())) {
		...
		fomrobject_t *tail = (fomrobject_t *)((UDATA)walk + objectSize - J9MODRON_HEAP_BYTES_PER_HEAPMAP_BIT);
...
fomrobject_t*
MM_WriteOnceCompactor::getForwardingPtr(fomrobject_t* objectPtr) const
{
	fomrobject_t *forwardPointer = NULL;
	...
	if (!region->_compactData._shouldCompact) {
		...
		fomrobject_t* targetBaseAddress = _compactTable[oldPageIndex].getAddr();
		if (NULL == targetBaseAddress) {
			...
			if ((newPageIndex == oldPageIndex) && (targetBaseAddress >= objectPtr)) {
				...
				if (isSliding) {
					...
					forwardPointer = (fomrobject_t *)((UDATA)targetBaseAddress + bytes);
				} else {
					/* this is an inter-page compaction */
					void *base = pageStart(oldPageIndex);
					UDATA totalSize = bytesAfterSlidingTargetToLocateObject(objectPtr, (fomrobject_t *)base);
					forwardPointer = (fomrobject_t *)((UDATA)targetBaseAddress + totalSize);
				}
			}
		}
	}
...
UDATA
MM_WriteOnceCompactor::bytesAfterSlidingTargetToLocateObject(fomrobject_t *oldPointer, fomrobject_t *slidingTarget) const
{
	UDATA totalSize = 0;
	...
	for (UDATA bias = 0; (((UDATA)base + bias) < (UDATA)oldPointer) && (bias < sizeof_page); bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
		...
		if ((slidingTarget > baseAddress) && (pageIndex((fomrobject_t *)baseAddress) == slidingIndex)) {
...
void
MM_WriteOnceCompactor::fixupMixedObject(MM_EnvironmentVLHGC* env, fomrobject_t *objectPtr, J9MM_FixupCache *cache)
{
	/* object may have moved so ensure that its class loader knows where it is */
	// OMRTODO _extensions->classLoaderRememberedSet->rememberInstance(env, objectPtr);
	
	GC_MixedObjectIterator it(_javaVM->omrVM, objectPtr);
	while (GC_SlotObject* slotObject = it.nextSlot()) {
		fomrobject_t *pointer = slotObject->readReferenceFromSlot();
		if (NULL != pointer) {
			fomrobject_t *forwardedPtr = getForwardWrapper(env, pointer, cache);
...
#if 0 OMRTODO
void
MM_WriteOnceCompactor::fixupClassObject(MM_EnvironmentVLHGC* env, fomrobject_t *classObject, J9MM_FixupCache *cache)
{
	...
	if (NULL != classPtr) {
		...
		do {
			...
			while (NULL != (slotPtr = classIterator.nextSlot())) {
				fomrobject_t *originalObject = *slotPtr;
				if (NULL != originalObject) {
					fomrobject_t *forwardedObject = getForwardWrapper(env, originalObject, cache);
			...
			classPtr->classObject = (fomrobject_t)classObject;
...
void
MM_WriteOnceCompactor::fixupClassLoaderObject(MM_EnvironmentVLHGC* env, fomrobject_t *classLoaderObject, J9MM_FixupCache *cache)
{
	fixupMixedObject(env, classLoaderObject, cache);
	
	J9ClassLoader *classLoader = J9VMJAVALANGCLASSLOADER_VMREF((OMR_VMThread*)env->getLanguageVMThread(), classLoaderObject);
	if (NULL != classLoader) {
		...
		volatile fomrobject_t * slotPtr = NULL;
		fomrobject_t* originalObject = NULL;
...
void
MM_WriteOnceCompactor::fixupPointerArrayObject(MM_EnvironmentVLHGC* env, fomrobject_t *objectPtr, J9MM_FixupCache *cache)
{
	/* object may have moved so ensure that its class loader knows where it is */
	// OMRTODO _extensions->classLoaderRememberedSet->rememberInstance(env, objectPtr);
	...
	GC_ArrayletObjectModel::ArrayLayout layout = _extensions->indexableObjectModel.getArrayLayout((fomrobject_t*)objectPtr);
		
	if (GC_ArrayletObjectModel::InlineContiguous == layout) {
		UDATA elementsToWalk = _extensions->indexableObjectModel.getSizeInElements((fomrobject_t*)objectPtr);
		...
		for (UDATA i = 0; i < elementsToWalk; i++) {
			...
			Assert_MM_true((0 == previous) || ((address + sizeof(fomrobject_t)) == previous));
			previous = address;
		}
		...
		UDATA numArraylets = _extensions->indexableObjectModel.numArraylets((fomrobject_t*)objectPtr);
		...
		if (NULL != GC_PointerArrayIterator(_javaVM, objectPtr).nextSlot()) {
			...
			UDATA totalElementCount = _extensions->indexableObjectModel.getSizeInElements((fomrobject_t*)objectPtr);
			UDATA externalArrayletCount = _extensions->indexableObjectModel.numExternalArraylets((fomrobject_t*)objectPtr);
			UDATA fullLeafSizeInBytes = _javaVM->arrayletLeafSize;
			UDATA elementsPerFullLeaf = fullLeafSizeInBytes / sizeof(fomrobject_t);
...
void
MM_WriteOnceCompactor::fixupObject(MM_EnvironmentVLHGC* env, fomrobject_t *objectPtr, J9MM_FixupCache *cache)
{
...
MM_WriteOnceCompactor::tagArrayletLeafRegionsForFixup(MM_EnvironmentVLHGC *env)
{
	...
	while (NULL != (region = regionIterator.nextRegion())) {
		...
		if (region->isArrayletLeaf()) {
			...
			fomrobject_t *spineObject = region->_allocateData.getSpine();
...
public:
		...
		, _heapTop(heapTop)
	{
		...
		// OMRTODO setIncludeStackFrameClassReferences(false);
	}
	...
	virtual void doSlot(fomrobject_t** slot)
	{
		fomrobject_t *pointer = *slot;
		if ((pointer >= _heapBase) && (pointer < _heapTop)) {
			fomrobject_t *newPointer = _compactScheme->getForwardingPtr(pointer);
		...
		if (0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
			...
			if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				if (NULL != classLoader->classLoaderObject) {
					...
					fomrobject_t *classLoaderObject = classLoader->classLoaderObject;
...
MM_WriteOnceCompactor::rebuildMarkbitsInRegion(MM_EnvironmentVLHGC *env, MM_HeapRegionDescriptorVLHGC *region)
{
	...
	fomrobject_t *high = (fomrobject_t *)region->getHighAddress();
...
public:
	...
	virtual void doClass(J9Class *clazz)
	{
		...
		while (volatile fomrobject_t * slot = classIterator.nextSlot()) {
			/* discard volatile since we must be in stop-the-world mode */
			doSlot((fomrobject_t*)slot);
		}
	}
...
void
MM_WriteOnceCompactor::verifyHeapObjectSlot(fomrobject_t* object)
{
...
void
MM_WriteOnceCompactor::verifyHeapMixedObject(fomrobject_t* objectPtr)
{
	GC_MixedObjectIterator it(_javaVM->omrVM, objectPtr);
	while (GC_SlotObject* slotObject = it.nextSlot()) {
...
void
MM_WriteOnceCompactor::verifyHeapArrayObject(fomrobject_t* objectPtr)
{
	GC_PointerArrayIterator it(_javaVM, objectPtr);
	while (GC_SlotObject* slotObject = it.nextSlot()) {
		verifyHeapObjectSlot(slotObject->readReferenceFromSlot());
...
MM_WriteOnceCompactor::verifyHeap(MM_EnvironmentVLHGC *env, bool beforeCompaction) {
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		...
		while (fomrobject_t* objectPtr = markedObjectIterator.nextObject()) {
 ...
 */
void
MM_WriteOnceCompactor::updateInternalLeafPointersAfterCopy(fomrobject_t *destinationPtr, fomrobject_t *sourcePtr)
{
...
MM_WriteOnceCompactor::fixupArrayletLeafRegionSpinePointers()
{
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		...
		if (NULL != spine) {
			...
			fomrobject_t *newSpine = (fomrobject_t *)getForwardingPtr((fomrobject_t *)spine);
			if (newSpine != spine) {
...
MM_WriteOnceCompactor::fixupArrayletLeafRegionContentsAndObjectLists(MM_EnvironmentVLHGC* env)
{
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		if (region->_compactData._shouldFixup) {  
			...
			fomrobject_t* slotPointer = (fomrobject_t*)region->getLowAddress();
			fomrobject_t* endOfLeaf = (fomrobject_t*)region->getHighAddress();
			while (slotPointer < endOfLeaf) {
				/* TODO: 4096 elements is an arbitrary number */
				fomrobject_t* endPointer = slotPointer + 4096;
				if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
					while (slotPointer < endPointer) {
						GC_SlotObject slotObject(_javaVM->omrVM, slotPointer);
						fomrobject_t *pointer = slotObject.readReferenceFromSlot();
						if (NULL != pointer) {
							fomrobject_t *forwardedPtr = getForwardingPtr(pointer);
							slotObject.writeReferenceToSlot(forwardedPtr);
...
MM_WriteOnceCompactor::planRegion(MM_EnvironmentVLHGC *env, MM_HeapRegionDescriptorVLHGC *subAreaRegion, UDATA postMoveRegionSizeInBytes, UDATA *objectCount, UDATA *byteCount, UDATA *skippedObjectCount)
{
	...
	while (targetSpaceRequired > 0) {
		...
		if (NULL == targetPointer) {
			...
			doPlanSlide(env, sourceRegionBase, (fomrobject_t *)copyStart, (fomrobject_t *)sourceRegionTop, objectCount, &bytesConsumedInMove);
			*byteCount += bytesConsumedInMove;
			...
			UDATA bytesConsumedInMove = 0;
			copyStart = doPlanEvacuate(env, &freeChunk, topEdge, (fomrobject_t *)copyStart, (fomrobject_t *)sourceRegionTop, objectCount, &bytesConsumedInMove);
			*byteCount += bytesConsumedInMove;
			Assert_MM_true(bytesConsumedInMove <= targetSpaceRequired);
...
fomrobject_t*
MM_WriteOnceCompactor::doPlanEvacuate(MM_EnvironmentVLHGC *env, void **copyDestinationBase, void *copyDestinationTop, fomrobject_t *firstToCopy, fomrobject_t *endOfCopyBlock, UDATA *objectCount, UDATA *byteCount)
{
	...
	fomrobject_t *compactDestination = (fomrobject_t *)*copyDestinationBase;
	...
	fomrobject_t *firstObjectNotEvacuated = NULL;
	for (IDATA page = startPage; (NULL == firstObjectNotEvacuated) && (page < finishPage); page++) {
		...
		if (0 == estimatedSizeOfPage) {
			...
			compactDestination = (fomrobject_t *)((UDATA)compactDestination + estimatedSizeOfPage);
...
void
MM_WriteOnceCompactor::doPlanSlide(MM_EnvironmentVLHGC *env, void *copyDestinationBase, fomrobject_t *firstToCopy, fomrobject_t *endOfCopyBlock, UDATA *objectCount, UDATA *byteCount)
{
	/* we use the MM_HeapMapWordIterator in this function so ensure that it is safe (can't be tested during init since assertions don't work at that point) */
	Assert_MM_true(0 == (sizeof_page % J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP));
	/* the compact destination starts at the deadObject we were given but will be bumped along as we compact.  We need to save this back to deadObject, at the end, since it is an in/out parameter */
	...
	for (objectPtr = markedObjectIterator.nextObject(); NULL != objectPtr; objectPtr = nextObject) {
		...
		compactDestination = (fomrobject_t*)((UDATA)compactDestination + postMoveObjectSize);
...
MM_WriteOnceCompactor::rebuildMarkMapInMovingPage(MM_EnvironmentVLHGC *env, MM_MarkMap *markMap, void *pageBase)
{
	...
	bool skipTail = false;
	IDATA index = pageIndex((fomrobject_t *)pageBase);
	fomrobject_t *targetBase = _compactTable[index].getAddr();
	fomrobject_t *originalTarget = targetBase;
	IDATA originalTargetIndex = pageIndex(originalTarget);
	UDATA slotIndex = markMap->getSlotIndex((fomrobject_t *)pageBase);
	fomrobject_t *previousHead = NULL;
	...
	for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
		...
		if (skipTail) {
			fomrobject_t *tail = iterator.nextObject();
			if (NULL != tail) {
				UDATA size = ((UDATA)tail - (UDATA)previousHead) + (_compactTable[index].isGrowBitSet(previousHead) ? _objectAlignmentInBytes : 0);
				targetBase = (fomrobject_t *)((UDATA)targetBase + size + J9MODRON_HEAP_BYTES_PER_HEAPMAP_BIT);
				skipTail = false;
			}
		}
		while (NULL != (object = iterator.nextObject())) {
			...
			fomrobject_t *newLocation = targetBase;
...
MM_WriteOnceCompactor::fixupFinalizableObjects(MM_EnvironmentVLHGC *env)
{
...

	{
		...
		while (NULL != referenceObject) {
			fomrobject_t* forwardedPtr = getForwardingPtr(referenceObject);
			/* read the next link out of the moved copy of the object before we add it to the buffer */
...
void
MM_WriteOnceCompactor::fixupFinalizableList(MM_EnvironmentVLHGC *env, fomrobject_t headObject)
{
	GC_FinalizableObjectBuffer objectBuffer(_extensions);

	while (NULL != headObject) {
		fomrobject_t* forwardedPtr = getForwardingPtr(headObject);
		/* read the next link out of the moved copy of the object before we add it to the buffer */
			...
			if(OMR_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
				...
				while(NULL != (segment = segmentIterator.nextSegment())) {
					...
					while(NULL != (clazz = classHeapIterator.nextClass())) {
						Assert_MM_true(!OMR_ARE_ANY_BITS_SET(clazz->classDepthAndFlags, J9_JAVA_CLASS_DYING));
						Assert_MM_true(!OMR_ARE_ANY_BITS_SET(J9CLASS_EXTENDED_FLAGS(clazz), J9ClassGCScanned));
						fomrobject_t* classObject = clazz->classObject;
						Assert_MM_true(NULL != classObject);
		...
		while((classLoader = classLoaderIterator.nextSlot()) != NULL) {
			if(OMR_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
				...
				while(NULL != (segment = segmentIterator.nextSegment())) {
					...
					while(NULL != (clazz = classHeapIterator.nextClass())) {
						Assert_MM_true(!OMR_ARE_ANY_BITS_SET(clazz->classDepthAndFlags, J9_JAVA_CLASS_DYING));
						if(OMR_ARE_ANY_BITS_SET(J9CLASS_EXTENDED_FLAGS(clazz), J9ClassGCScanned)) {
							fomrobject_t* classObject = clazz->classObject;
							Assert_MM_true(NULL != classObject);
