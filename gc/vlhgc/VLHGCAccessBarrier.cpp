...
MM_VLHGCAccessBarrier::newInstance(MM_EnvironmentBase *env)
{
	...
	barrier = (MM_VLHGCAccessBarrier *)env->getForge()->allocate(sizeof(MM_VLHGCAccessBarrier), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (barrier) {
 ...
 */
void
MM_VLHGCAccessBarrier::postObjectStore(OMR_VMThread *vmThread, fomrobject_t *destObject, fomrobject_t *destAddress, fomrobject_t *value, bool isVolatile)
{
 ...
 */
void
MM_VLHGCAccessBarrier::postObjectStore(OMR_VMThread *vmThread, J9Class *destClass, fomrobject_t **destAddress, fomrobject_t *value, bool isVolatile)
{
	fomrobject_t destObject = J9VM_J9CLASS_TO_HEAPCLASS(destClass);
...
bool 
MM_VLHGCAccessBarrier::preBatchObjectStore(OMR_VMThread *vmThread, fomrobject_t *destObject, bool isVolatile)
{
...
bool 
MM_VLHGCAccessBarrier::preBatchObjectStore(OMR_VMThread *vmThread, J9Class *destClass, bool isVolatile)
{
	fomrobject_t destObject = J9VM_J9CLASS_TO_HEAPCLASS(destClass);
 ...
 */
void
MM_VLHGCAccessBarrier::postObjectStoreImpl(OMR_VMThread *vmThread, fomrobject_t *dstObject, fomrobject_t *srcObject)
{
 ...
 */
void 
MM_VLHGCAccessBarrier::preBatchObjectStoreImpl(OMR_VMThread *vmThread, fomrobject_t *dstObject)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(vmThread);
	_extensions->cardTable->dirtyCard(env, dstObject);
}
...
I_32
MM_VLHGCAccessBarrier::backwardReferenceArrayCopyIndex(OMR_VMThread *vmThread, fomrobject_t *srcObject, fomrobject_t *destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(vmThread);
	...
	if (_extensions->indexableObjectModel.isInlineContiguousArraylet(destObject)) {
		...
		if (((MM_IncrementalGenerationalGC *)_extensions->getGlobalCollector())->isGlobalMarkPhaseRunning()) {
			_extensions->cardTable->dirtyCard(env, (fomrobject_t *)destObject);
		}
	}
 ...
 */
I_32
MM_VLHGCAccessBarrier::forwardReferenceArrayCopyIndex(OMR_VMThread *vmThread, fomrobject_t *srcObject, fomrobject_t *destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(vmThread);
	I_32 retValue = ARRAY_COPY_NOT_DONE;
 ...
 */
void 
MM_VLHGCAccessBarrier::recentlyAllocatedObject(OMR_VMThread *vmThread, fomrobject_t *dstObject) 
{ 
	/* TODO: we may need to revive this if we apply VMDESIGN 2048 to Tarok specs */
}

void 
MM_VLHGCAccessBarrier::postStoreClassToClassLoader(OMR_VMThread *vmThread, J9ClassLoader* destClassLoader, J9Class* srcClass)
{
	fomrobject_t* classLoaderObject = destClassLoader->classLoaderObject;
	...
	if (NULL == classLoaderObject) {
		...
		fomrobject_t* classObject = J9VM_J9CLASS_TO_HEAPCLASS(srcClass);
		postObjectStoreImpl(vmThread, classLoaderObject, classObject);
	}
}

void*
MM_VLHGCAccessBarrier::jniGetPrimitiveArrayCritical(OMR_VMThread* vmThread, jarray array, jboolean *isCopy)
{
	void *data = NULL;
	OMR_VM *javaVM = vmThread->javaVM;
	J9InternalVMFunctions *functions = javaVM->internalVMFunctions;
	VM_VMAccess::inlineEnterVMFromJNI(vmThread);

	fomrobject_t *arrayObject = (fomrobject_t*)J9_JNI_UNWRAP_REFERENCE(array);
	...
	if (shouldCopy) {
		...
		arrayObject = (fomrobject_t*)J9_JNI_UNWRAP_REFERENCE(array);
...
void
MM_VLHGCAccessBarrier::jniReleasePrimitiveArrayCritical(OMR_VMThread* vmThread, jarray array, void * elems, jint mode)
{
	OMR_VM *javaVM = vmThread->javaVM;
	J9InternalVMFunctions *functions = javaVM->internalVMFunctions;
	VM_VMAccess::inlineEnterVMFromJNI(vmThread);
...
const jchar*
MM_VLHGCAccessBarrier::jniGetStringCritical(OMR_VMThread* vmThread, jstring str, jboolean *isCopy)
{
	jchar *data = NULL;
	OMR_VM *javaVM = vmThread->javaVM;
	J9InternalVMFunctions *functions = javaVM->internalVMFunctions;
	...
	fomrobject_t *stringObject = (fomrobject_t*)J9_JNI_UNWRAP_REFERENCE(str);
	fomrobject_t *valueObject = (fomrobject_t*)J9VMJAVALANGSTRING_VALUE(vmThread, stringObject);
	bool shouldCopy = false;
	...
	if (shouldCopy) {
		...
		if (NULL == data) {
			...
			if (isCompressed) {
				...
				for (i = 0; i < length; i++) {
					data[i] = (jchar)J9JAVAARRAYOFBYTE_LOAD(vmThread, (fomrobject_t)valueObject, i) & (jchar)0xFF;
				}
			} else {
				GC_ArrayObjectModel* indexableObjectModel = &_extensions->indexableObjectModel;
				
				if (OMR_ARE_ANY_BITS_SET(javaVM->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
...
void
MM_VLHGCAccessBarrier::jniReleaseStringCritical(OMR_VMThread* vmThread, jstring str, const jchar* elems)
{
	OMR_VM *javaVM = vmThread->javaVM;
	J9InternalVMFunctions *functions = javaVM->internalVMFunctions;
	VM_VMAccess::inlineEnterVMFromJNI(vmThread);
