...
MM_OwnableSynchronizerObjectBuffer::MM_OwnableSynchronizerObjectBuffer(MM_GCExtensionsBase* extensions, UDATA maxObjectCount)
	: MM_BaseVirtual()
...
void
MM_OwnableSynchronizerObjectBuffer::add(MM_EnvironmentBase* env, omrobjectptr_t object)
{
	...
	if ( (_objectCount < _maxObjectCount) && _region->isAddressInRegion(object) ) {
		...
		flush(env);
		// OMROTODO _extensions->accessBarrier->setOwnableSynchronizerLink(object, NULL);
		_head = object;
		_tail = object;
		_objectCount = 1;
