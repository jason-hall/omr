...
MM_Wildcard *
MM_Wildcard::newInstance(MM_GCExtensionsBase *extensions, U_32 matchFlag, const char* needle, UDATA needleLength, char* pattern)
{
	MM_Wildcard *wildcard = (MM_Wildcard*)extensions->getForge()->allocate(sizeof(MM_Wildcard), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (NULL != wildcard) {
...
void
MM_Wildcard::kill(MM_GCExtensionsBase *extensions)
{
	tearDown(extensions);
...
bool
MM_Wildcard::initialize(MM_GCExtensionsBase *extensions)
{
	return true;
}

void
MM_Wildcard::tearDown(MM_GCExtensionsBase *extensions)
{
	OMRPORT_ACCESS_FROM_OMRVM(extensions->getOmrVM());
