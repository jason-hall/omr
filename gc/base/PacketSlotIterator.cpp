...
MM_PacketSlotIterator::nextSlot()
{
	...
	bool found = false;
	while ((!found) && (_nextSlot < (omrobjectptr_t *)_packet->_currentPtr)) {
	...
	omrobjectptr_t *slotToReturn = NULL;
	if (_nextSlot < (omrobjectptr_t *)_packet->_currentPtr) {
...
void
MM_PacketSlotIterator::resetSplitTagIndexForObject(omrobjectptr_t correspondingObject, UDATA newValue)
{
	omrobjectptr_t *tagSlot = _nextSlot - 2;
	if (tagSlot >= (omrobjectptr_t *)_packet->_basePtr) {
		...
		if (PACKET_ARRAY_SPLIT_TAG == (PACKET_ARRAY_SPLIT_TAG & (UDATA)*tagSlot)) {
			...
			omrobjectptr_t *objectSlot = _nextSlot - 1;
			Assert_MM_true(correspondingObject == *objectSlot);
			*tagSlot = (omrobjectptr_t)newValue;
		}
	}
}
