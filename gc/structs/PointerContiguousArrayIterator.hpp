...
{
private:
	omrarrayptr_t _arrayPtr;	/**< pointer to the array object being iterated */
	GC_SlotObject _slotObject;		/**< Create own SlotObject class to provide output */

	fomrobject_t *_scanPtr;			/**< pointer to the current array slot */
	fomrobject_t *_endPtr;			/**< pointer to the array slot where the iteration will terminate */
...
public:
	MMINLINE void initialize(omrobjectptr_t objectPtr)
	{
		...
		_arrayPtr = (omrarrayptr_t)objectPtr;
		
		/* Set current and end scan pointers */
		_endPtr = ((fomrobject_t *)extensions->indexableObjectModel.getDataPointerForContiguous(_arrayPtr)) - 1;
		_scanPtr = _endPtr + extensions->indexableObjectModel.getSizeInElements((omrobjectptr_t)_arrayPtr);
	...
	MMINLINE omrobjectptr_t getObject()
	{
		return (omrobjectptr_t)_arrayPtr;
	}
