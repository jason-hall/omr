...
protected:
	...
	omrobjectptr_t _objectPtr;		/**< save address of object this class initiated with */
...
public:
	 ...
	 */
	MMINLINE void 
	initialize(omrobjectptr_t objectPtr)
	{
		_objectPtr = objectPtr;
		// OMRTODO J9Class *clazzPtr = J9GC_J9OBJECT_CLAZZ(objectPtr);
		//initializeDescriptionBits(clazzPtr);

		/* Set current and end scan pointers */
		_scanPtr = (fomrobject_t *)(objectPtr + 1);
		_endPtr = _scanPtr; // OMRTODO _endPtr = (fomrobject_t *)((U_8*)_scanPtr +clazzPtr->totalInstanceSize);
	}
	 ...
	 */
	MMINLINE void advanceToSlot(fomrobject_t *slotPtr)
	{
	 ...
	 */
	GC_MixedObjectIterator (OMR_VM *omrVM, omrobjectptr_t objectPtr)
		: _slotObject(GC_SlotObject(omrVM, NULL))
		, _objectPtr(NULL)
		, _scanPtr(NULL)
		, _endPtr(NULL)
