...
private:
	...
	fomrarray_t *_currentArrayletBaseAddress; /* The base address of the current arraylet */
...
protected:
public:
	MMINLINE void initialize(fomrobjectptr_t objectPtr) {
		MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(_javaVM->omrVM);

		fomrarray_t *arrayPtr = (fomrarray_t *)objectPtr;
		...
		if(0 == index) {
			...
			fomrarray_t *arrayoidPointer = extensions->indexableObjectModel.getArrayoidPointer(arrayPtr);
	...
	GC_PointerArrayletInlineLeafIterator(OMR_VM *omrVM)
		: _slotObject(GC_SlotObject(javaVM->omrVM, NULL))
		, _omrVM(omrVM)
	{
		_arrayletLeafSize = _omrVM->arrayletLeafSize;
		_fobjectsPerLeaf = _arrayletLeafSize/sizeof(fomrobject_t);
	}
