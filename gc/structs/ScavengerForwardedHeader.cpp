...
MM_ScavengerForwardedHeader::validateAssumptions()
{
	...
	// OMRTODO Assert_MM_true(offsetof(J9IndexableObjectContiguous, size) == offsetof(MutableHeaderFields, overlap));
...
MM_ScavengerForwardedHeader::setForwardedObject(omrobjectptr_t destinationObjectPtr)
{
	...
	// OMRTODO Assert_MM_true(0 == (offsetof(J9Object, clazz) % sizeof(UDATA)));

	Assert_MM_false(isForwardedPointer());

	// OMRTODO volatile MutableHeaderFields* objectHeader = (volatile MutableHeaderFields *)_objectPtr;
	// OMRTODO UDATA oldValue = *(UDATA *)&_preserved.clazz;
	...
	// OMRTODO UDATA newValue = (UDATA)destinationObjectPtr | FORWARDED_TAG;
...
MM_ScavengerForwardedHeader::didObjectGrowOnCopy()
{
	...
	return false; // OMRTODO (GROW_TAG == ((UDATA)_preserved.clazz & GROW_TAG));
}
