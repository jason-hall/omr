...
public:
...
#elif defined(J9VM_GC_HYBRID_ARRAYLETS)
	:_contiguousArrayIterator(omrVM)
	,_pointerArrayletIterator(omrVM)
#else
	...
	GC_PointerArrayIterator(OMR_VM *omrVM)
#if !defined(J9VM_GC_ARRAYLETS)
	  :_contiguousArrayIterator(omrVM)
#elif defined(J9VM_GC_HYBRID_ARRAYLETS)
	...
	MMINLINE void initialize(OMR_VM *omrVM, fomrobject_t *objectPtr)
	{
		...
		_contiguous = extensions->indexableObjectModel.isInlineContiguousArraylet(objectPtr);
		if (_contiguous) {
	...
	MMINLINE omrobjectptr_t getObject()
	{
		...
		return (omrobjectptr_t)objectPtr;
	}
