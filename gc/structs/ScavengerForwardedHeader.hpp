...
// OMRTODO Is this even used? #include "ObjectAccessBarrier.hpp"
...
protected:
	...
	struct MutableHeaderFields {
		/* slot must be always aligned to UDATA */
		fomrobject_t slot;
...
public:
	...
	isForwardedPointer()
	{
		return true; // OMRTODO FORWARDED_TAG == ((UDATA)_preserved.clazz & FORWARDED_TAG);
	}
	...
	isIndexable()
	{
		// OMRTODO J9Class* preservedClass = (J9Class*)((UDATA)_preserved.clazz & J9GC_J9OBJECT_CLAZZ_ADDRESS_MASK);
		return true; //return J9GC_CLASS_IS_ARRAY(preservedClass);
	}
	...
	hasBeenMoved()
	{
		return true; // OMRTODO return (OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS == ((UDATA)_preserved.clazz & OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS));
	}
	...
	hasBeenHashed()
	{
		return true; // OMRTODO return (0 != ((UDATA)_preserved.clazz & (OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS | OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS)));
	}
	...
	{
		return true; // OMRTODO return (OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS == ((UDATA)_preserved.clazz & OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS));
	}
	...
	{
		return true; // OMRTODO return J9_GC_MULTI_SLOT_HOLE == ((UDATA)_preserved.clazz & J9_GC_OBJ_HEAP_HOLE_MASK);
	}
	 ...
	 */
	MMINLINE void* // OMRTODO J9Class*
	getPreservedClass()
	{
		Assert_MM_false(isForwardedPointer());
		return NULL; // OMRTODO return (J9Class*)((UDATA)_preserved.clazz & J9GC_J9OBJECT_CLAZZ_ADDRESS_MASK);
	}
	...
	getPreservedFlags()
	{
		return 0; // OMRTODO return (UDATA)_preserved.clazz & J9GC_J9OBJECT_CLAZZ_FLAGS_MASK;
	}
	...
	{
		return 0; // OMRTODO return (UDATA)_preserved.clazz & OBJECT_HEADER_AGE_MASK;
	}
	/**
	...
	MM_ScavengerForwardedHeader(omrobjectptr_t object) :
		_objectPtr(object)
	{
		// OMRTODO volatile MutableHeaderFields* originalHeader = (volatile MutableHeaderFields *)_objectPtr;
		 ...
		 */
		// OMRTODO *(UDATA *)&_preserved.clazz = *((UDATA *)&originalHeader->clazz);
	}
	...
	{
		// OMRTODO volatile MutableHeaderFields* newHeader = (volatile MutableHeaderFields *)copiedObjectPtr;

		/* class slot is always aligned to UDATA (compressed or not)
		 * so forwarded pointer is stored in UDATA word starts class slot address
		 * (overlap next slot for compressed)
		 */
		// OMRTODO *(UDATA *)&newHeader->clazz = *(UDATA *)&_preserved.clazz;
	}
	/**
	 * Update the new version of this object after it has been copied. This undoes any damaged
	 * caused by installing the forwarding pointer into the original prior to the copy.
	...
	fixupCopiedObject(omrobjectptr_t copiedObjectPtr, UDATA newAge, bool setOldFlag)
	{
		...
		// OMRTODO newHeader->clazz = (j9objectclass_t)(((UDATA)_preserved.clazz & ~OBJECT_HEADER_AGE_MASK) | newAge);

#if defined (J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
		/* first object slot is destroyed, must be restored from local copy as well */
		// OMRTODO newHeader->overlap = _preserved.overlap;
#endif /* defined (J9VM_INTERP_COMPRESSED_OBJECT_HEADER) */
	}
	...
	getPreservedIndexableSize()
	{
		...
		U_32 size = 0;// OMRTODO ((J9IndexableObjectContiguous *)_objectPtr)->size;
#endif /* defined (J9VM_INTERP_COMPRESSED_OBJECT_HEADER) */
	...
	MMINLINE I_32 computeObjectHash(OMR_VMThread* vmThread)
	{
		// OMRTODO J9JavaVM* javaVM = vmThread->javaVM;
		// OMRTODO return convertValueToHash(javaVM, (UDATA)_objectPtr);
		return 0;
	}

	MMINLINE UDATA
	getHashcodeOffset(OMR_VM* omrVM, omrobjectptr_t dest)
	{
		// OMRTODO MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(omrVM);
		UDATA hashOffset = 0;

		if (isIndexable()) {
			// OMRTODO hashOffset = extensions->indexableObjectModel.getHashcodeOffset((J9IndexableObject *)dest);
		} else {
			// OMRTODO hashOffset = extensions->mixedObjectModel.getHashcodeOffset(dest);
		}
...
protected:
	...
	getForwardedObjectNoCheck()
	{
		...
		// OMRTODO U_32 low = _preserved.clazz & ~ALL_TAGS;
		// OMRTODO U_32 high = _preserved.overlap;
		// OMRTODO omrobjectptr_t forwardedObject = (omrobjectptr_t)((U_64)low | ((U_64)high << 32));
		...
		// OMRTODO omrobjectptr_t forwardedObject = (omrobjectptr_t)(*(UDATA *)(&_preserved.clazz) & ~ALL_TAGS);

#endif /* defined (J9VM_INTERP_COMPRESSED_OBJECT_HEADER) && !defined(J9VM_ENV_LITTLE_ENDIAN) */

		return NULL; // OMRTODO forwardedObject;
	}
