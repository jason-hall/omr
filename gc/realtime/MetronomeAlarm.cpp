...
void MM_RTCAlarm::describe(OMRPortLibrary *port, char *buffer, I_32 bufferSize) {
	OMRPORT_ACCESS_FROM_OMRPORT(port);
 	omrstr_printf(buffer, bufferSize, "RTC  (Period = %.2f us Frequency = %d Hz)", 1.0/_osInterface->_extensions->RTC_Frequency, _osInterface->_extensions->RTC_Frequency);
}

void MM_HRTAlarm::describe(OMRPortLibrary *port, char *buffer, I_32 bufferSize) {
	OMRPORT_ACCESS_FROM_OMRPORT(port);
	 omrstr_printf(buffer, bufferSize, "High Resolution Timer (Period = %d us)", _osInterface->_extensions->hrtPeriodMicro);
}

void MM_ITAlarm::describe(OMRPortLibrary *port, char *buffer, I_32 bufferSize) {
	OMRPORT_ACCESS_FROM_OMRPORT(port);
	omrstr_printf(buffer, bufferSize, "Interval Timer (Period = %d us)", _osInterface->_extensions->itPeriodMicro);
}
