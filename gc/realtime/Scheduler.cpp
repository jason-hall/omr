...
MM_Scheduler::tearDown(MM_EnvironmentBase *env)
{
	...
	// OMRTODO if (NULL != _barrierSynchronization) {
...
MM_Scheduler::initialize(MM_EnvironmentBase *env)
{
	...
	// OMRTODO _barrierSynchronization = MM_BarrierSynchronization::newInstance(env);
...
MM_Scheduler::continueGC(MM_EnvironmentRealtime *env, GCReason reason, uintptr_t resonParameter, OMR_VMThread *thr, bool doRequestExclusiveVMAccess)
{
	...
	// OMRTODO _barrierSynchronization->preRequestExclusiveVMAccess(thr);
	...
	if (_exclusiveVMAccessRequired) {
		 ...
		 */
		// OMRTODO if (FALSE == _barrierSynchronization->requestExclusiveVMAccess(_threadWaitingOnMasterThreadMonitor, FALSE /* do not block */, &gcPriority)) {
		//	didGC = false;
		//	goto exit;
		//}
...
exit:
	...
	// OMRTODO _barrierSynchronization->postRequestExclusiveVMAccess(thr);
...
MM_Scheduler::waitForMutatorsToStop(MM_EnvironmentRealtime *env)
{
	...
	if (_masterThreadMustShutDown && _mode != WAKING_GC) {
		uintptr_t gcPriority = 0;
		// OMRTODO _barrierSynchronization->requestExclusiveVMAccess(env, TRUE /* block */, &gcPriority);
		_gc->setGCThreadPriority(env->getOmrVMThread(), gcPriority);
	}
	...
	// OMRTODO _barrierSynchronization->waitForExclusiveVMAccess(env, _exclusiveVMAccessRequired);
...
MM_Scheduler::startMutators(MM_EnvironmentRealtime *env) {
	...
	// OMRTODO _barrierSynchronization->releaseExclusiveVMAccess(env, _exclusiveVMAccessRequired);
}
