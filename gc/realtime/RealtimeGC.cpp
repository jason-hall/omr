...
MM_RealtimeGC::reportSyncGCEnd(MM_EnvironmentBase *env)
{
...
#endif /* defined(OMR_GC_DYNAMIC_CLASS_UNLOADING) */
	UDATA weakReferenceCount = _extensions->globalGCStats.metronomeStats._weakReferenceStats._cleared;
	UDATA softReferenceCount = _extensions->globalGCStats.metronomeStats._softReferenceStats._cleared;
	...
	UDATA phantomReferenceCount = _extensions->globalGCStats.metronomeStats._phantomReferenceStats._cleared;
...
MM_RealtimeGC::reportGCCycleEnd(MM_EnvironmentBase *env)
{
	 ...
	 */
	_vm->_gcCycleOn = 0;
	omrthread_monitor_notify_all(_vm->_gcCycleOnMonitor);
