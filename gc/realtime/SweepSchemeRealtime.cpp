...
MM_SweepSchemeRealtime::preSweep(MM_EnvironmentBase *env)
{
	...
	if (ext->isConcurrentSweepEnabled()) {
		...
		// OMRTODO _scheduler->_barrierSynchronization->releaseExclusiveVMAccess(env, _scheduler->_exclusiveVMAccessRequired);
	}
}
...
MM_SweepSchemeRealtime::postSweep(MM_EnvironmentBase *env)
{
	MM_GCExtensionsBase *ext = env->getExtensions();
	if (ext->isConcurrentSweepEnabled()) {
		// OMRTODO _scheduler->_barrierSynchronization->acquireExclusiveVMAccess(env, _scheduler->_exclusiveVMAccessRequired);
